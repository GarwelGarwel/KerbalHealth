...
namespace KerbalHealth
{
    class MicrogravityFactor : HealthFactor
    {
        ...
        public override double ChangePerDay(ProtoCrewMember pcm)
        {
            ...
            if (pcm.KerbalRef == null) Core.Log("MicrogravityFactor.ChangePerDay: pcm.KerbalRef is null for " + pcm.name + "!", Core.LogLevel.Important);
            else if (pcm.KerbalRef.InVessel == null) Core.Log("MicrogravityFactor.ChangePerDay: pcm.KerbalRef.InVessel is null for " + pcm.name + ", he/she is " + (Core.KerbalHealthList.Find(pcm).IsOnEVA ? "" : "NOT") + " on EVA!", Core.KerbalHealthList.Find(pcm).IsOnEVA ? Core.LogLevel.Debug : Core.LogLevel.Error);
